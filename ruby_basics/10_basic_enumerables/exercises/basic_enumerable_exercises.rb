...
def find_word_lengths(word_list)
  ...
  word_list.reduce(Hash.new(0)) do |result, word|
    ...
    # this is required to add to the hash every time, without it the result from the first iteration is an integer and the program breaks
    # puts result
    #result
  end
end

find_word_lengths(['one', 'two', 'three'])